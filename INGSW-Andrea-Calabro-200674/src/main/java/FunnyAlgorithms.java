public class FunnyAlgorithms {
	...
	/**
	 * Binary Search Ã¨ un algoritmo di ricerca per trovare la posizione di un
	...
	public void selectionSort(int[] array, int order) {
		...
		if (order != 0 && order != 1) {
			throw new IllegalArgumentException("L'ordine puÃ² essere 0 o 1.");
		}
	...
	public int stringToIntConverter(String number) throws UnsupportedOperationException {
		...
		if(non)
			throw new UnsupportedOperationException("Non è convertibile in numero");
		else {
			if(number.charAt(0)=='-') 
				nome=-(Integer.parseInt(number.substring(1)));
			else
				nome=(Integer.parseInt(number));
			
		if(nome<-32768 || nome>32767)
		throw new UnsupportedOperationException("Non è compreso nell'intervallo");
		else
		return nome;
		}
	}
